...
Which in turn is opensourced from https://github.com/Ovid/ovid.github.com
...
var map = {
    ...
    rotate_angle:     0.006,                // angle per rotation in radians
    ...
    select_star: function(event) {
        ...
        //alert(image_path);
        this.selected = -1;
        $('star_id').update('No System selected');
        for(var i=0; i<this.stars.length; i++) {
            star = this.stars[i];
            ...
            {
                this.selected = i;
                var image_path='/img/Planets/'+star.name+'.png';
                if (star.wormholes == 0) {
                    $('star_pic').setStyle({backgroundImage: 'url(' + image_path + ')'});
                    $('star_id').update("Name: "+star.name+'<br />Type: '+star.type);
                }
                else {
                    $('star_pic').setStyle({backgroundImage: 'url(' + image_path + ')'});
                    $('star_id').update(
                        '<p class="lestarstyle">' + star.name +'</p>'
                        + 'Type: '      + star.type
                        + '<br />Primary Terrain: ' + star.terrain
                        + '<br />Hyperspace Travel Time: '  + star.parsecs +' Hours'
                        + '<br /><div class="descript">' + star.desc+'</div>'
                    );
                }
                break;
            }
...
function star(x, y, z, name, type, terrain, num_parsecs, reachable, current, desc) {
    this.x         = x;
    this.y         = y;
    this.z         = z;
    this.name      = name;
